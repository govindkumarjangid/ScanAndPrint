  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-gradient-to-br from-amber-400 to-amber-500 rounded-3xl p-6 sm:p-7 text-stone-900 shadow-md flex items-center justify-between gap-4">
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-extrabold uppercase tracking-wider bg-stone-900/10 px-3 py-0.5 rounded-full w-max">
              Counter Display
            </span>
            <h3 className="text-xl font-extrabold font-heading">Print Your Shop QR Code</h3>
            <p className="text-xs text-stone-800 font-medium">Download high-res printable PDF poster for your shop counter.</p>
          </div>
          <Link to="/owner/qr-code" className="shrink-0">
            <button className="bg-stone-900 text-white font-extrabold px-5 py-3 rounded-2xl text-xs flex items-center gap-1.5 shadow-md hover:bg-stone-800 cursor-pointer">
        ...
        <div className="bg-gradient-to-br from-brand to-rose-600 rounded-3xl p-6 sm:p-7 text-white shadow-md flex items-center justify-between gap-4">
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-extrabold uppercase tracking-wider bg-white/20 px-3 py-0.5 rounded-full w-max">
              Windows PC App
            </span>
            <h3 className="text-xl font-extrabold font-heading">Print Agent (.exe)</h3>
            <p className="text-xs text-rose-100 font-medium">Download background software for 1-click silent hardware printing.</p>
          </div>
          <Link to="/owner/agent" className="shrink-0">
            <button className="bg-white text-brand font-extrabold px-5 py-3 rounded-2xl text-xs flex items-center gap-1.5 shadow-md hover:bg-rose-50 cursor-pointer">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-stone-200/80 shadow-xs flex flex-col gap-5">
        <div className="flex items-center justify-between border-b border-stone-100 pb-4">
          ...
          <Link to="/owner/jobs" className="text-xs font-extrabold text-brand hover:underline flex items-center gap-1">
            ...
            <tbody className="divide-y divide-stone-100">
              {recentJobs.map((j) => (
                <tr key={j.id} className="hover:bg-stone-50/60 transition-colors">
                  <td className="py-3.5 px-4 font-bold text-stone-900 font-mono text-xs">{j.id}</td>
                  <td className="py-3.5 px-4 font-semibold text-stone-800">{j.file}</td>
                  ...
                  <td className="py-3.5 px-4 font-extrabold text-stone-900">₹{j.amount}</td>
                  <td className="py-3.5 px-4">
