...
export default function OwnerPrinters() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl w-full">
      ...
      {/* Main Card */}
      <form onSubmit={handleSavePrinters} className="bg-white rounded-3xl p-5 sm:p-8 border border-stone-200/80 shadow-xs flex flex-col gap-6 overflow-hidden">
        ...
        <div className={`p-4 sm:p-5 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
          ...
          <div className="flex items-start sm:items-center gap-3.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {connectedPrinters.length > 0 && (
                  <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-800 text-[10px] font-extrabold px-2 py-0.5 rounded-full shrink-0">
          ...
          <button
            ...
            disabled={isScanning}
            className="btn btn-outline bg-white btn-sm font-bold! flex items-center justify-center gap-2 self-start sm:self-auto shrink-0 shadow-xs w-full sm:w-auto"
          >
        ...
        {isScanning && connectedPrinters.length === 0 ? (
          ...
          <div className="bg-stone-50/70 p-4 rounded-2xl border border-stone-200/80 flex flex-col gap-2 overflow-hidden">
            <span className="text-xs font-extrabold uppercase tracking-wider text-stone-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {connectedPrinters.map((p, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2.5 bg-white p-3 rounded-xl border border-stone-200 text-xs font-semibold text-stone-800 shadow-2xs min-w-0"
                >
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-extrabold uppercase tracking-wider text-stone-700 flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <span>Black & White Printer Device</span>
            {bwPrinter && (
              <span className="text-stone-400 font-mono text-[11px] font-medium lowercase truncate max-w-full">
                selected: {bwPrinter}
              </span>
            )}
          </label>
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-extrabold uppercase tracking-wider text-brand flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <span>Color Printer Device</span>
            {colorPrinter && (
              <span className="text-rose-400 font-mono text-[11px] font-medium lowercase truncate max-w-full">
